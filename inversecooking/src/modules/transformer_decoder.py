...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, ingr_features, ingr_mask, incremental_state, img_features, device='cpu'):
...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, ingr_features, ingr_mask, captions, img_features, incremental_state=None, device='cpu'):
        ...
        for p, layer in enumerate(self.layers):
            x = layer(x, ingr_features, ingr_mask, incremental_state, img_features, device=device)
    ...
    def sample(self, ingr_features, ingr_mask, greedy=True, temperature=1.0, beam=-1,
               img_features=None, first_token_value=0, replacement=True, last_token_value=0, device='cpu'):
        ...
        for i in range(self.seq_length):
            ...
            outputs, _ = self.forward(ingr_features, ingr_mask, torch.stack(sampled_ids, 1),
                                      img_features, incremental_state, device=device)
    ...
    def sample_beam(self, ingr_features, ingr_mask, beam=3, img_features=None, first_token_value=0,
                   replacement=True, last_token_value=0, device='cpu'):
        k = beam
        alpha = 0.0
        # create dummy previous word
        if ingr_features is not None:
