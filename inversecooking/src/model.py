...
def mask_from_eos(ids, eos_value, mult_before=True, device='cpu'):
...
class InverseCookingModel(nn.Module):
    ...
    def sample(self, img_inputs, greedy=True, temperature=1.0, beam=-1, true_ingrs=None, device='cpu'):
        ...
        if not self.recipe_only:
            ingr_ids, ingr_probs = self.ingredient_decoder.sample(None, None, device=device, greedy=True,
                                                                  temperature=temperature, beam=-1,
                                                                  img_features=img_features, first_token_value=0,
                                                                  replacement=False)

            # mask ingredients after finding eos
            sample_mask = mask_from_eos(ingr_ids, eos_value=0, mult_before=False, device=device)
        ...
        ids, probs = self.recipe_decoder.sample(input_feats, input_mask, greedy, temperature, beam, img_features, 0,
                                                last_token_value=1, device=device)
